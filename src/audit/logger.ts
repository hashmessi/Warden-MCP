...
export class AuditLogger {
  ...
  public static canonicalize(
    ...
    return JSON.stringify({ action, actor, subjectHash, details, prevHash });
  }
  ...
  public static async appendLog(params: CreateAuditEntryParams): Promise<AuditEntry> {
    const subjectHash = params.subject ? this.hashIdentifier(params.subject) : null;
    ...
    const newHash = crypto.createHash("sha256").update(canonicalData).digest("hex");
